...
def recipe_cover_delete(sender, instance, *args, **kwargs):
    old_instance = Recipe.objects.filter(id=instance.id).first()
    cover_place_holder = "place_holder.png"
    name_cover = instance.cover.name
    if not name_cover == cover_place_holder:
